...
class ValidityTester(object):   # 异步检测类，可以对给定的代理的可用性进行异步检测。
    test_api = TEST_API
...
class PoolAdder(object):  # 代理添加器，用来触发爬虫模块，对代理池内的代理进行补充，代理池代理数达到阈值时停止工作。
    """
    add proxy to pool
    """
...
class Schedule(object):  # 代理池启动类，运行RUN函数时，会创建两个进程，负责对代理池内容的增加和更新。
    @staticmethod
